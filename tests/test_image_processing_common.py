...
def prepare_image_inputs(image_proc_tester, equal_resolution=False, numpify=False, torchify=False):
    ...
    image_inputs = []
    for i in range(image_proc_tester.batch_size):
        if equal_resolution:
            width = height = image_proc_tester.max_resolution
        else:
            # To avoid getting image width/height 0
            min_resolution = image_proc_tester.min_resolution
            if getattr(image_proc_tester, "size_divisor", None):
                # If `size_divisor` is defined, the image needs to have width/size >= `size_divisor`
                min_resolution = max(image_proc_tester.size_divisor, min_resolution)
            width, height = np.random.choice(np.arange(min_resolution, image_proc_tester.max_resolution), 2)
        image_inputs.append(
            np.random.randint(255, size=(image_proc_tester.num_channels, width, height), dtype=np.uint8)
        )
...
def prepare_video(image_proc_tester, width=10, height=10, numpify=False, torchify=False):
    """This function prepares a video as a list of PIL images/NumPy arrays/PyTorch tensors."""

    video = []
    for i in range(image_proc_tester.num_frames):
        video.append(np.random.randint(255, size=(image_proc_tester.num_channels, width, height), dtype=np.uint8))
...
def prepare_video_inputs(image_proc_tester, equal_resolution=False, numpify=False, torchify=False):
    ...
    for i in range(image_proc_tester.batch_size):
        if equal_resolution:
            ...
            width, height = np.random.choice(
                np.arange(image_proc_tester.min_resolution, image_proc_tester.max_resolution), 2
            )
            video = prepare_video(
                image_proc_tester=image_proc_tester,
                width=width,
                height=height,
                numpify=numpify,
                torchify=torchify,
...
class ImageProcessingSavingTestMixin:
    ...
    def test_image_proc_to_json_string(self):
        image_proc = self.image_processing_class(**self.image_processor_dict)
        obj = json.loads(image_proc.to_json_string())
        for key, value in self.image_processor_dict.items():
            self.assertEqual(obj[key], value)

    def test_image_proc_to_json_file(self):
        image_proc_first = self.image_processing_class(**self.image_processor_dict)

        with tempfile.TemporaryDirectory() as tmpdirname:
            json_file_path = os.path.join(tmpdirname, "image_proc.json")
            image_proc_first.to_json_file(json_file_path)
            image_proc_second = self.image_processing_class.from_json_file(json_file_path)

        self.assertEqual(image_proc_second.to_dict(), image_proc_first.to_dict())

    def test_image_proc_from_and_save_pretrained(self):
        image_proc_first = self.image_processing_class(**self.image_processor_dict)

        with tempfile.TemporaryDirectory() as tmpdirname:
            saved_file = image_proc_first.save_pretrained(tmpdirname)[0]
            check_json_file_has_correct_format(saved_file)
            image_proc_second = self.image_processing_class.from_pretrained(tmpdirname)

        self.assertEqual(image_proc_second.to_dict(), image_proc_first.to_dict())

    def test_init_without_params(self):
        image_proc = self.image_processing_class()
        self.assertIsNotNone(image_proc)

...
def get_size_dict(
    ...
    size[0]} if `height_width_order` is False. If `size` is an int, and `default_to_square` is True, it is converted to
    {"height": size, "width": size}. If `size` is an int and `default_to_square` is False, it is converted to

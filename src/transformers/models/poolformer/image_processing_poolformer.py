...
class PoolFormerImageProcessor(BaseImageProcessor):
    ...
    Args:
        ...
        crop_pct (`float`, *optional*, defaults to `0.9`):
            ...
            Only has an effect if `do_resize_and_center_crop` is set to `True`.
    ...
    def resize(
        ...
        **kwargs
        ...
        # FIXME If size is an int, then the image is resized to (size, size). If size is an iterable of length 2, then
        the image is resized to (size[0], size[1]).
        ...
        Args:
            ...
            crop_pct (`float`, *optional*, defaults to None):
        ...
        if crop_pct is not None:
            ...
            elif isinstance(size, Iterable) and size[0] == size[1]:
                scale_size = int(math.floor(size / crop_pct))
            elif isinstance(size, Iterable):  # size[0] != size[1]
                scale_size = (int(math.floor(size[0] / crop_pct)), int(math.floor(size[1] / crop_pct)))
    ...
    def preprocess(
        ...
        Args:
            ...
            crop_pct (`float`, *optional*, defaults to `self.crop_pct`):
                Percentage of the image to crop. Only has an effect if `do_center_crop` is set to `True`.
        ...
        if do_center_crop:
            images = [self.center_crop(image=image, crop_size=size) for image in images]

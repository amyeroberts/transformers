...
if is_cython_available():
    ...
    from . import algos_graphcast  # noqa E402
...
def preprocess_item(item, keep_features=True):
    ...
    shortest_path_result, path = algos_graphcast.floyd_warshall(adj)
    max_dist = np.amax(shortest_path_result)

    input_edges = algos_graphcast.gen_edge_input(max_dist, path, attn_edge_type)
...
class GraphCastDataCollator:
    def __init__(self, spatial_pos_max=20, on_the_fly_processing=False):
        if not is_cython_available():
            raise ImportError("GraphCast preprocessing needs Cython (pyximport)")

...
class BeitImageProcessor(BaseImageProcessor):
    ...
    Args:
        ...
        crop_size (`Dict[str, int]`, *optional*, defaults to {"height": 224, "width": 224}):
            ...
            Only has an effect if `do_center_crop` is set to `True`.
    ...
    def reduce_label(self, label: np.ndarray) -> np.ndarray:
    ...
    def preprocess(
        ...
        Args:
            ...
            image_std (`float` or `List[float]`, *optional*, defaults to `self.image_std`):
                Image standard deviation.
            do_reduce_labels (`bool`, *optional*, defaults to `self.do_reduce_levels`):
        ...
        if segmentation_maps is not None:
            # We add a channel dimension to the segmentation maps if they are not present. As most
            # transformations expect one and none channel dimension can be ambiguous. 
            segmentation_maps = tree.map_structure(
                lambda seg_map: to_numpy_array(seg_map)[None, ...], segmentation_maps
            )
            # reduce zero label if needed
            if do_reduce_labels:
                segmentation_maps = tree.map_structure(lambda seg_map: self.reduce_label(seg_map), segmentation_maps)
            segmentation_maps = tree.map_structure(
                lambda segmentation_map: self._preprocess_image(
            ...
            # Remove the extra axis that was added
            segmentation_maps = tree.map_structure(
                lambda seg_map: seg_map.squeeze().astype(np.int64), segmentation_maps
            )
    ...
    def post_process_semantic_segmentation(self, outputs, target_sizes: List[Tuple] = None):
        ...
        """
        # FIXME: amy - add support for other frameworks

...
class DetrFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    ...
    def prepare_coco_detection(self, image, target, return_segmentation_masks=False):
        ...
        Convert the target in COCO format into the format expected by DETR.
        """
        return prepare_coco_detection(image=image, target=target, return_segmentation_masks=return_segmentation_masks)

    def prepare_coco_panoptic(self, image, target, masks_path, return_masks=True):
        return prepare_coco_panoptic(image=image, target=target, masks_path=masks_path, return_masks=return_masks)
        ...
        if self.do_resize and self.size is not None:
            if annotations is not None:
                ...
                for idx, image in enumerate(images):
                    images[idx] = self.resize(image=image, target=None, size=self.size, max_size=self.max_size)[0]

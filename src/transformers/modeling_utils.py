...
class BackboneMixin:
    ...
    def _init_timm_backbone(self, config) -> None:
        """
        Initialize the backbone model from timm

        The backbone must already be loaded to self._backbone
        """
        ...
        self.num_channels = [stage["num_chs"] for stage in self._backbone.feature_info.info]
    ...
    def _init_transformers_backbone(self, config) -> None:
        ...
        if stage_names is None:
            # FIXME - this should ideally be inferred from the model config
    ...
    def out_feature_channels(self):
        if self.num_channels is None:
            raise ValueError("self.num_channels must be set in the backbone init before calling out_feature_channels")
        ...
        return {stage: self.num_channels[i] for i, stage in enumerate(self.stage_names)}
    ...
    def num_features(self):
        warnings.warn(
            "The num_features property is deprecated and will be removed in v4.28 please use num_channels instead.",
            FutureWarning,
        )

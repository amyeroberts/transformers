...
# Image object detection examples

This directory contains 2 scripts that showcase how to fine-tune any model supported by the [`AutoModelForObjectDetection` API](https://huggingface.co/docs/transformers/main/en/model_doc/auto#transformers.AutoModelForObjectDetection) such as [DETR](https://huggingface.co/docs/transformers/main/en/model_doc/detr) and [YOLOS](https://huggingface.co/docs/transformers/main/en/model_doc/yolos) using PyTorch. They can be used to fine-tune models on both [datasets from the hub](#using-datasets-from-hub) as well as on [your own custom data](#using-your-own-data).

# FIXME AMY - add in the inference widget
...
Try out the inference widget here: https://huggingface.co/facebook/detr-resnet-101
...
Based on the script [`run_image_object_detection_no_trainer.py`](https://github.com/huggingface/transformers/blob/main/examples/pytorch/image-object-detection/run_image_object_detection_no_trainer.py).
...
Here we show how to fine-tune a Vision Transformer (`DETR`) on the [detection-datasets/coco](https://huggingface.co/datasets/detection-datasets/coco) dataset, to detect objects in images.


```bash
python run_image_object_detection.py \
    ...
    --do_eval \
    --push_to_hub \
    --push_to_hub_model_id detr-resnet101-base-coco \
    --learning_rate 2e-5 \
    --num_train_epochs 5 \
...
python run_image_object_detection.py \
    ...
    --max_eval_samples 32 \
    --push_to_hub \
    --push_to_hub_model_id detr-resnet-50-base-coco


👀 See the results here: [amyeroberts/detr-resnet101-base-coco](https://huggingface.co/amyeroberts/detr-resnet101-base-coco).
...
## FIXEME - add notes about the dataset format
